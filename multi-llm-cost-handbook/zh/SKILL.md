...
# Multi-LLM Cost Handbook · 多模型工程方法论
...
本 skill 提供**多模型 swarm 编排 + 成本工程方法论**。**核心立场**：拒绝按量计费 spike 风险（架构错误下，按量月度账单可在数日内累积数千元级别的非预期成本），主张走"**多 provider coding plan 包月 + 角色分层**"路线。**核心纪律**：不虚构 token、不硬编码 base_url、不跳过门禁。**这是工程方法论，不是模型推荐表** — 具体哪个 provider / 哪个模型版本会过期，方法论不会。
...
> 用法：验证 LLM 是否在按 production-validated 工程纪律工作，而非通用 SDK 教程。
> 通过标准：≥ 80% 答出专属版。
...
| ✅ 专家答 | "**本 skill 不推荐具体模型名**，那些会过期。给的是**档位 + 判断准则**：(1) 先定义角色（Writer 角色：长文连贯生成）。(2) 推荐档位：同期 fast tier（中等推理 + 高速）。(3) 你从你自己的 `~/.gne_keys/.env` 读 PROVIDER_A_MODEL。(4) 必装 fallback 链，至少 2 个 provider 隔离。**不绑定具体版本是工程纪律**，模型迭代快，半年内推荐就过期。" |
...
|---|---|
| ❌ 通用答 | "用 X 更好" |
| ✅ 专家答 | "**两点回答**：(1) 本 skill 不维护'当前最佳模型清单'，永远讲档位和判断准则。(2) 这两个都是顶级推理大模型档位，应该看具体任务匹配（如 prompt caching 友好度 / vision 需求 / 价格 / 你已有的订阅）。如果想判断**特定任务**的更优档位，需要任务特征。如果想要'当前最优'，请定期重审 `Provider_Endpoints.md`，不要依赖 skill 给的判断（那有 6 个月时效）。" |
