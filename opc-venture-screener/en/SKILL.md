...
This skill is the **decision-gate** node of the full OPC chain (opportunity discovery → venture screening → commercialization → execution governance). For the minimal protocols of pre/post stages, see [`references/composite-workflows.md`](./references/composite-workflows.md).

The main SKILL.md retains only the decision-gate protocol; pre/post stages are self-contained but lazy-loaded. **No stage is exempt from the 15 red lines.**
